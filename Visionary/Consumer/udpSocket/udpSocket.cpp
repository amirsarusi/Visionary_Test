...
SocketException::SocketException(const std::string &message, bool inclSysMsg) throw() : userMessage(message)
{
    if (inclSysMsg)
    {
        userMessage.append(": ");
...
void Socket::setLocalAddressAndPort(const std::string &localAddress,unsigned short localPort) noexcept(false)
{
...
void CommunicatingSocket::connect(const std::string &foreignAddress,unsigned short foreignPort) noexcept(false)
{
    // Get the address of the requested host
...
std::string CommunicatingSocket::getForeignAddress() noexcept(false)
{
    sockaddr_in addr;
    unsigned int addr_len = sizeof(addr);
...
UDPSocket::UDPSocket() noexcept(false) : CommunicatingSocket(SOCK_DGRAM,IPPROTO_UDP)
{
    setBroadcast();
}
...
UDPSocket::UDPSocket(const std::string &localAddress, unsigned short localPort) noexcept(false) : CommunicatingSocket(SOCK_DGRAM, IPPROTO_UDP)
{
...
void UDPSocket::sendTo(const void *buffer, int bufferLen,const std::string &foreignAddress, unsigned short foreignPort) noexcept(false)
{
    sockaddr_in destAddr;
    fillAddr(foreignAddress, foreignPort, destAddr);
...
int UDPSocket::recvFrom(void *buffer, int bufferLen, std::string &sourceAddress,unsigned short &sourcePort) noexcept(false)
{
